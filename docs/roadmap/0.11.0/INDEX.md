...
- 構想來源：原 [backlog/player-overreach-adjudication.md](../backlog/player-overreach-adjudication.md)（出貨後已刪；**契約以本 INDEX＋docs 為準**）
...
| 行為 | 0.10.0 現行 | 0.11.0 |
|------|-------------|--------|
| `POST /api/turn` | 一律進對局 GM → Writer | 先輕量／必要時深層；PASS 才進對局 GM；pending 時再送故事句＝放棄原句並走新句閘門 |
| 過強句 | 模型常順從，events 落地 | 不進對局 session／Writer，直到終態 |
| UI | 送出立刻寫玩家氣泡；busy「處理中」；busy 全鎖含回主頁 | 另右側抽屜分頁：(1) GM (2) Debug（僅 `config.debug`）；待決故事輸入不鎖、提示可改寫；`#log` 玩家氣泡／`chat_tail` **僅**故事 GM 成功後寫；GM 側欄送出立刻顯示玩家句 |
...
5. **故事 vs GM 頻道。** 裁決與勸說不進 `#log` 的 narration／NPC 氣泡。玩家氣泡與 `chat_tail` **僅**在故事 GM **已成功**（輕量／深審 PASS，或終態 (1)(2)）後寫入 `#log`。pending、硬拒、僅側欄 (3) 不得留下該句玩家氣泡；若 client 已樂觀插入則須移除。pending 的 HTTP 200 **沒有** `gm` 鍵。開場隱含句仍不顯示玩家氣泡。hydrate 只還原已落盤故事，不把 `pending.json` 的原文灌進 `#log`。待決：**不**鎖故事 `#input`／`#send`；輸入列提示固定繁中 **「上一句待 GM 裁決。於此送出新句即放棄並改寫。」**（非 busy、無 spinner、禁止與「處理中」兩套疊加）。`#log` **不**插「待判決」假氣泡。待決 **不**鎖 `#go-home`／`#delete-world`。`#restart` 可點但**不清** pending、不把未裁決句寫回 `#log`。GM 側欄：玩家送出後 **立刻**顯示該則玩家句，不必等 GM 回應。0.9.0 已定案 2「submit 立刻寫玩家氣泡」與已定案 13 busy 全鎖：本版在閘門／pending 路徑覆寫如上；PASS 且無 pending、或待決後送出新故事句正在處理時，仍可顯示 busy「處理中」。
6. **側欄。** 預設收成小 icon，不佔主欄。玩家可手動開收。僅深審 `discuss` 時自動展開；終態後自動收起。終態後該場 GM 文字 **只留本頁 client 記憶**（可手動再打開看最後一則）；**不**另存磁碟側欄尾。F5 且非 pending 時 `gm_chat.messages` 為空。請玩家選終態時，用 **GM 文字** 列出 1／2／3（說服／切分／重輸），玩家自由回覆；**不**做三顆必選按鈕（可選輔助鈕但非門檻）。純說明／回答的 `talk` **不得**把解釋編成 (1)(2)(3)；編號只給「請選一條路」。(1)(2)(3) 必須是那三條終態選項，不是說明大綱。深審第一則：先散文說明過線原因，再另起三行選項。
7. **側欄三條終態。** (1) 說服：可多輪；接受則程式寫 `player-memory` 後以 **原** `player_text` 跑故事 GM。(2) 切分：故事 GM 帶約束；`events[]` 只反映成立段／失敗嘗試，禁止把過線寫成已發生。(3) 重輸：側欄 `revise` 丟原句、不解進對局、等玩家再打；**或**待決時在故事欄送出新句（丟原句並立刻以新句走完整閘門）。深審第一次 `pass` 不開側欄。`pass_original` 且 `player_memory_patch` 空白或僅空白字元 → 當 `talk`（保持 pending，請補承認內容），禁止無新事實就放行。`split_constraint` 為 **該拍** `GmContext` 可選字串，不落 `gm_note`、不進對玩家 HTTP、不跨拍自動帶入。
...
11. **Pending。** 存該 uuid（HOW 檔）。F5／同 process hydrate 若仍 pending：提示可改寫、側欄展開並還原 GM 訊息。pending 時 **對外** `POST /api/turn` **不是** 409：視為放棄未決（刪 pending、dispose meta、bump abandon，與定案 17 同守衛），再以**新句**走完整閘門（硬拒／輕量／深審）。內部 (1)(2) 的 `runTurn(..., { fromPending: true })` 不走這條放棄。亦可 `POST /api/gm-chat` 推進 meta。回主頁／刪世界／load 他檔須清本場 `pending.json`。新 process `bootWorlds` 在刪 pointer **之前**刪當時 pointer uuid 的 `pending.json`（與 home 相同：放棄未決）。
...
- [x] 無害故事句：無側欄自動展開；Writer／對局 session 行為與 0.10.0 等價（mock 可測）。
- [x] 虛構過線句（如瞬殺全場）：不寫 `events` 進 KB、對局 jsonl 無該 user 原文、無該句 `chat_tail`／`#log` 玩家氣泡，直到 (1)(2) 終態；(3) 則永不進。
- [x] 待決：`GET /api/state` 可還原 pending；故事輸入可送；提示「上一句待 GM 裁決。於此送出新句即放棄並改寫。」；無 `#log` 假氣泡與未裁決玩家句；無第二套 busy；`#go-home`／`#delete-world` 可點。
- [x] `POST /api/turn` 於 pending → 放棄原句並以新句走完整閘門（不再 409 `adjudication_pending`）。
- [x] 側欄 GM 文字含 1／2／3 意涵；玩家回覆可 `talk` 多輪。
- [x] (1) 接受：`player-memory/current.json` body 含承認內容；隨後故事 GM 跑 **同一則** 原句；側欄收起。空 patch 不得解鎖。
- [x] (2)：故事輸出存在；落盤 events 不含「過線已成功」；`split_constraint` 不進 `gm_note`／對玩家 HTTP；側欄收起。
- [x] (3)：無新 episode／無該句對局 session／無該句 `#log` 玩家氣泡；故事輸入解鎖。
- [x] 深審 PASS（輕量誤報）：不開側欄、故事正常一拍（深審 schema 為 `decision`，不是 disposition）。
- [x] 契約攻擊／未成年人：不開 (1)(2) 談判；不落盤；HTTP 200＋`hard_reject`＋`notice`、無 `gm`；(1)(2) skip 後仍硬拒。
- [x] 輕量／深審壞 JSON 或失敗：不得 fall through 當 pass（輕量→escalate，深審→discuss）。
- [x] `buildGmContext` 含 `player_memory`；turn HTTP **無** 該欄。
- [x] Default／custom setup 有空 `player-memory/current.json`（或等價缺檔當空且下一寫入建檔）。
- [x] 刪世界後該 uuid 目錄（含 `player-memory/`）不在。
- [x] `bootWorlds` 清當時 pointer 的 `pending.json`；回主頁刪 pending、保留 `player-memory/current.json`。
- [x] `bun test` 全綠；出貨文件與 backlog 清理。
- [x] 瀏覽器：主路徑 + 過線 + 重輸至少各走一次。（出貨同意；mock／窄測已覆蓋主路徑）
...
- **做：** icon／右側滑入浮層（GM／可選 Debug 分頁）；自動開收；待決提示可改寫且故事輸入不鎖；hydrate pending；`#log` 玩家氣泡僅故事成功後寫；GM 側欄送出立刻顯示玩家句；gm-chat 等待勿 `setTurnBusy` 全鎖；回主頁／刪世界可點；重開畫面不清 pending。
- **不做：** 華麗 IM。
